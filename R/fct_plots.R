...
output_player_game_plot <- function(input_df, input_season, input_weeks, input_player, input_metric, input_threshold) {
  ...
  p1 <- ggplot2::ggplot(player_plot_data,
    ...
    ggplot2::geom_text(ggplot2::aes(label = paste0("EPA/P: ", round(.data$EPAP,2))), size = 2.5, fontface = "bold", nudge_y = -max_y/8, color = "#6c0000") +
    ggplot2::geom_text(ggplot2::aes(label = paste0("SR: ", round(.data$SR,2))), size = 2.5, fontface = "bold", nudge_y = -max_y/12, color = "#6c0000") +
